...
class EnableKioskModeReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        if (intent?.action == SocketService.ACTION_SOCKET) {
            ...
            Log.i("Broadcast", "Modo kiosco Fuerada")
        }
    }
}
        ...
        btnSend.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                if (textCodeInput == storeCode) {
                    ...
                    setKioskPolicies(false, isAdmin())
                    Toast.makeText(this@FirsAppActivity, "Código correcto", Toast.LENGTH_LONG).show()
                } else {
                    Toast.makeText(this@FirsAppActivity, "Código incorrecto", Toast.LENGTH_LONG).show()
                }
            }
        }
...
//        setImmersiveMode(enable)
    ...
    private fun setRestrictions(disallow: Boolean) {
        ...
        devicePolicyManager.setStatusBarDisabled(mcomponentName, disallow)
    }

    private fun setUserRestriction(restriction: String, disallow: Boolean) = if (disallow) {
        devicePolicyManager.addUserRestriction(mcomponentName, restriction)

    } else {
        devicePolicyManager.clearUserRestriction(mcomponentName, restriction)
    }
    ...
    private fun setLockTask(start: Boolean, isAdmin: Boolean) {
        if (isAdmin) {
            if (start) {
                ...
                devicePolicyManager.setLockTaskPackages(mcomponentName, emptyArray())
                finish()
    ...
    private fun setUpdatePolicy(enable: Boolean) {
        if (enable) {
            ...
            devicePolicyManager.setSystemUpdatePolicy(mcomponentName, null)
        }
    }
